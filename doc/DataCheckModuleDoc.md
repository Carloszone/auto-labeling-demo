...
- `spike`:  1, 代表出现瞬时突变的帧
- `step`: 2, 代表出现台阶跳变的帧
- `oscillation`: 3, 代表出现振荡现象的帧
- `end_zero`: 4， 代表末端执行器全0速的帧
- `end_step`: 5, 代表出现末端执行器的动作参数出现台阶跳变的帧
    ...
    2: [
        {
            ...
            "desc": "检测发现速度({topic})存在瞬时突变"
        },
        {
            "anomaly_code": 5,
            "anomaly_name": "spike",
            "topic": "left_arm_state",
            "desc": "检测发现末端执行器动作({topic})出现台阶跳变"
        },
        {
            "anomaly_code": 2,
            "anomaly_name": "blur",
            "topic": "right_image",
            "desc": "检测发现图像({topic})模糊帧"
        },
        ...
    ],
    ...
...
- `anomaly_name`: 异常类型名称
- `topic`: 发现异常的topic名称
- `desc`: 异常描述，格式为f"检测发现{数据类型}({topic名称}){异常类型名称}"
- `start_timestamp_ns`: 异常区间的起点时间戳(纳秒)
- `start_timestamp_sec`: 异常区间的起点时间戳(秒)
- `start_timestamp_index`: 异常区间的起点时间戳在`timestamp_list`中的索引
- `end_timestamp_ns`: 异常区间的终点时间戳(纳秒)
- `end_timestamp_sec`: 异常区间的终点时间戳(秒)
...
12. 缺少 `parser_info`、`timestamp_list` 为空、列表长度不一致、必需 schema key 缺失时，模块直接失败并返回明确错误。
13. 非必需 topic 无法检测、滑动窗口帧数不足、单帧图像无法解码等可退化场景不会中断整个任务，并会记录 warning。
14. 当 `data_detection` 或 `image_detection` 关闭时，对应检测结果为空，但模块仍能输出合法结构。
15. 所有失败和 warning 日志都包含 `task_id`、`job_id`、检测器名称、topic、帧 index、`timestamp_ns` 和错误原因。
