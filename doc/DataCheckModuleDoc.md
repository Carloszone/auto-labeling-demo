...
当前阶段，本模块的输入有两个来源：
1. 来源于编排层处理后的 `parser_info`：`timestamp_list`、`video_paths`、`state_list`、`action_list`、`state_vector_list`、`action_vector_list`、`state_schema`、`action_schema`。独立调用模块时仍兼容 Parser 原始 `image_list`。
...
- `params.min_duration_sec`: 最小状态持续时间，单位秒。该参数同时约束 ruptures 候选分段和 HSMM 状态持续时间，默认 1 秒，内部按 30 FPS 换算为 30 帧
...
- `basic.task_id` 缺失。
- `basic.parser_info` 缺失，或不包含 `timestamp_list`、`state_list`、`action_list`、`state_schema`、`action_schema`，或同时缺少 `video_paths` 和兼容输入 `image_list`。
- `timestamp_list` 为空。
- `timestamp_list` 不是按时间单调递增排列。
- `timestamp_list`、`state_list`、`action_list` 长度不一致；兼容输入 `image_list` 存在时，其长度也必须一致。
...
- 2026-07-16：突变检测和图像检测的默认 `z_score` 统一调整为 `5`。
