...
MVP 阶段先聚焦本地 MCAP 文件输入，不考虑远程下载、前端交互、数据库任务管理和并发调度。核心流程需要完成解析、质检、事件节点生成和事件标注，最终输出可供训练数据清洗、人工复核和后续数据转换使用的标注 JSON。
...
5. 事件标注：将事件片段中的视频图像传入 VLM 模型，结合 prompt 输出事件标签和描述，并生成最终标注 JSON。
...
5. 事件标注阶段：将事件节点之间的视频图像传入 VLM 模型，结合 prompt 生成事件标签和描述，并输出最终标注 JSON。
...
8. 事件标注模块能输出每个事件的标签和描述字段，并生成最终标注 JSON。
