...
- 页面目标：快速处理一个连续任务对应的一个或多个 MCAP，将多摄像头视频、自动标注 event 和异常区间可视化，并支持简单人工复核和 JSON 导出
- MVP 范围：每个工作项上传 1–32 个 MCAP、单任务串行处理、本地临时文件、一个当前工作项和最近 5 次成功结果的进程内状态，不使用数据库和 S3
- 部署方式：前端和后端部署在同一台服务主机，其他主机通过 HTTP 地址访问页面、上传文件并启动自动标注
- 工作项命名：`job<雪花 ID>`，即字符串 `job` 与雪花 ID 直接拼接
- 保留规则：新建工作项时，已成功的当前工作项进入历史；失败或未完成的旧工作项不保留。历史最多 5 条，超出后删除最早成功结果及其临时目录

一次输入的多个 MCAP 被视为同一连续动作的若干片段，属于同一个工作项。后端必须按 MCAP 内主时间 topic 的时间戳排序，不能依赖文件名或上传顺序；重叠时保留较早时间段文件的数据，小缺口补帧/插值，大缺口拒绝处理。
...
- 单个工作项允许同时上传 1～32 个 MCAP；页面选择顺序仅用于展示，不能决定处理顺序。
- 所有 MCAP 文件合计上限为 50 GiB。
- 前端通过 HTTP 将文件上传到后端。
- 后端流式写入本机临时目录，禁止把整个文件一次性读入内存。
- 上传过程中使用 `.part` 临时文件；上传和基础校验成功后再原子改名。
- 点击“上传并创建”后立即进入创建中状态并禁用自动标注；只有后端完成全部文件上传与基础校验、返回新 Job ID 后，页面才切换到该新工作项并允许运行。仅选择文件不会创建或切换 Job。
- MVP 不要求断点续传；上传中断后删除不完整文件，用户重新上传。

MVP 使用单次 `multipart/form-data` 请求，使用重复的 `mcaps` 字段提交多个文件，并同时提交 `robot_config`；算法配置和 VLM Prompt 在后续启动请求中提交。

multipart 字段为一个或多个 `mcaps` 和一个 `robot_config`。后端仍兼容旧版单文件 `mcap` 字段。接口、响应和错误结构见 `doc/BackendDoc.md`。

文件名可以带人工序号，上传列表也保持用户选择顺序，但服务不得据此直接拼接。运行前必须读取每个 MCAP 中 `main_time_topic` 的实际起止时间，并以记录时间戳决定处理顺序。页面只显示后端返回的实际排序和边界诊断结果，不提供排序、缺口阈值或重叠策略配置入口。

多 MCAP 的内部固定规则全部属于页面不可见、不可调整参数：30 FPS 全局相对时间轴、前文件优先的重叠裁剪、小缺口补帧/插值、大缺口失败，以及文件数量、连续间隔、视频补帧和运动插值阈值。
...
- 默认值来自后端 `app/core/defaults.py` 的 `DEFAULT_INPUT_PROMPT`，通过配置接口返回。
- 用户未填写时使用默认值。
- VLM 输入 Prompt 与 event 中的动作摘要 `prompt` 是两个不同字段。
- 页面提供可编辑展开项展示默认 System Prompt；用户可以修改或恢复默认值，运行请求显式传入当前内容，留空时由后端回退到默认值。

默认示例：

```text
这是一段机器人操作视频的关键帧序列，请描述机器人完成的任务，使用中文描述其进行的动作和环境细节信息。
```
...
| `data_check_config.data_detection.sudden_change_config.z_score` | `5` |
...
| `data_check_config.image_detection.z_score` | `5` |
...
| `event_labeling_config.sampling.params.sampling_frame_gap` | `5` |
...
- 进度按当前工作项的统一多 MCAP 流程计算，不按单个文件分别创建任务。
- 页面通过 Job ID 下拉框切换当前工作项及最近 5 次成功结果；历史项允许查看、复核和导出，但不允许重新运行。
...
- Event 复核区的 gripper 筛选与二级 Event 轨道联动；选择一个或多个 `trigger_topic_key` 后，轨道只显示由这些 gripper 变化节点生成的 Event。
...
- 输入或修改 VLM Prompt。
- 启动自动标注。
- 点击“新建”立即生成 draft Job ID，将已完成的当前工作项转入历史，并清空 MCAP/Robot Config 选择列表；之后上传文件只归属于该 draft。
- 通过 Job ID 下拉框切换最近 5 次成功标注结果；历史项保留展示、复核和导出，不允许重新运行。
...
- 按 `trigger_topic_key` 多选筛选 event；`baseline_camera_key` 仅标识 VLM 取帧所用摄像头。
...
2. 用户选择 1–32 个属于同一连续任务的 MCAP 和 robot config，前端通过 HTTP 上传文件内容。
...
├── input/segment_000.mcap
├── input/segment_001.mcap
...
- 创建新工作项时保留最近 5 个成功工作项目录，清理失败、未完成或超出数量限制的目录；服务正常退出时可以清理临时目录。
- 不使用数据库和 S3；历史仅保存在当前后端进程内，服务重启后不恢复。
...
- 上传文件合计超过 50 GiB、连接中断或临时磁盘空间不足时，任务上传失败并清理 `.part` 文件。

后端校验每个 `.mcap` 的扩展名及可读性，robot config 必须是 UTF-8 JSON。客户端文件名只用于显示，服务器按上传序号保存为安全的 `segment_NNN.mcap`，但后续处理顺序只依据文件内时间戳；配置固定保存为 `robot.json`。临时目录创建前检查至少 6 GiB 可用空间。

### 9.2 处理

- 运行中禁用重复启动。
- 创建新工作项时若当前任务仍在运行，必须先提示并确认。
- MCAP 解析、视频生成或流水线结构错误会使状态变为 `failed`，并展示失败阶段和错误摘要。
- 单个 event 的 VLM HTTP、超时、连接或响应解析异常不会中断流水线；该 event 输出 `action_state=-1`、错误摘要和详细错误，并继续处理后续 event。
...
- 上传：单次 multipart，MCAP 合计最大 50 GiB，不断点续传。
- 视频：PyAV/libx264 H.264 MP4，支持 HTTP Range。
- 状态：一个当前工作项和最近 5 个成功工作项的进程内记录，1 秒/5 秒轮询。
...
- 2026-07-16：修复大文件上传期间仍可运行旧 Job 的竞态；创建新工作项期间禁用自动标注，并明确页面在上传校验成功后才切换到新 Job。
- 2026-07-16：精简默认 User Prompt；System Prompt 改为有后端默认值且允许前端编辑、恢复默认和随运行请求传入。
- 2026-07-15：支持 1–32 个 MCAP、50 GiB 总上传量；按文件内主时间 topic 排序和拼接，生成固定 30 FPS 相对时间轴及 CFR MP4，页面展示一个当前工作项和最近 5 次成功结果。
- 2026-07-15：VLM 单 event 请求失败改为错误标注并继续流水线；补充多 MCAP 临时目录、历史保留和失败边界。
